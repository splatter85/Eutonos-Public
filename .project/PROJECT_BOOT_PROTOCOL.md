...
7. Inspect only the `User Introduction` section in `.project/CURRENT_STATE.md`. Follow the agent introduction protocol in `docs/TOVA_HELP.md` when the marker is missing or `not_shown`, or the user asks for help; do not automatically repeat it for `shown` or `skipped`. Read the rest of Current State only when broader project truth is needed.
